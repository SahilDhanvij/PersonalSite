...
const ProjectCard = React.forwardRef<HTMLDivElement, { project: Project; idx: number; highlighted?: boolean }>(
  ...
  return (
    <div ref={ref} className="group relative" onMouseEnter={() => setHover(true)} onMouseLeave={handleMouseLeave}>
      <div
        ref={cardRef}
        ...
        style={{
          ...
          transition: 'background-color 0.4s, border-color 0.4s, box-shadow 0.5s, transform 0.4s cubic-bezier(0.16, 1, 0.3, 1)',
          willChange: 'transform',
        }}
        onMouseMove={handleMouseMove}
      >
        <div className="flex items-start justify-between mb-4 relative z-10">
          <span className="font-display text-2xl font-700 select-none transition-colors duration-500"
          ...
          <svg className="w-4 h-4 transition-all duration-300"
            style={{ color: 'var(--accent)', opacity: isActive ? 0.8 : 0.25, transform: isActive ? 'translate(2px, -2px)' : 'none' }}
            fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 17L17 7M17 7H7M17 7v10" />
          </svg>
        </div>
        <div className="relative z-10">
          <h3 className="font-display text-lg font-700 mb-2 transition-colors duration-300"
            style={{ color: isActive ? 'var(--accent)' : 'var(--text)' }}>{project.title}</h3>
          <p className="text-sm leading-relaxed mb-5" style={{ color: 'var(--text-mid)' }}>{project.description}</p>
          <div className="flex flex-wrap gap-1.5">
            {project.tags.map(tag => (
              <span key={tag} className="font-mono text-[9px] tracking-wider uppercase px-2.5 py-1 rounded-full"
...
export const AllProjectsPage: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  useEffect(() => {
    ...
    const animate = (time: number) => {
      ...
      for (let i = 0; i < featured.length; i++) {
        ...
        if (hasActive) {
          if (i === activeIdxRef.current) { s *= 1.1; o = 1; }
          else { o = 0.25; }
        }
  ...
  const handleCardClick = (idx: number) => {
    ...
    setActiveIdx(idx); activeIdxRef.current = idx; pausedRef.current = true;
    ...
    resumeTimerRef.current = setTimeout(() => {
      setActiveIdx(-1); activeIdxRef.current = -1; pausedRef.current = false;
    }, 5000);
  };
  ...
  return (
    <div className="min-h-screen relative z-10" style={{ backgroundColor: 'var(--bg)' }}>
      ...
      <div className="sticky top-0 z-50 px-6 md:px-12 lg:px-24 py-4"
        style={{ backgroundColor: 'var(--nav-bg)', backdropFilter: 'blur(24px) saturate(1.2)', borderBottom: '1px solid var(--border-light)' }}>
      ...
      <div className="relative" style={{ height: '70vh', minHeight: '500px' }}>
        <div className="absolute inset-0 flex items-center justify-center overflow-hidden">
          <div className="text-center absolute top-8 left-0 right-0 z-20">
            <span className="font-mono text-[10px] tracking-[0.2em] uppercase" style={{ color: 'var(--text-light)' }}>
              ...
              {featured.map((project, idx) => (
                <div
                  ...
                  style={{
                    ...
                    width: '260px', willChange: 'transform', backfaceVisibility: 'hidden',
                  }}
                ...
                >
                  ...
                  >
                    <div className="flex items-center justify-between mb-3">
                      <span className="font-display text-xl font-700 select-none" style={{ color: 'var(--accent-medium)' }}>{project.image}</span>
                      <svg className="w-3.5 h-3.5" style={{ color: 'var(--accent)', opacity: 0.4 }} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 17L17 7M17 7H7M17 7v10" />
                      </svg>
                    </div>
                    <h3 className="font-display text-base font-700 mb-1.5" style={{ color: 'var(--text)' }}>{project.title}</h3>
      ...
      <div className="max-w-[1400px] mx-auto px-6 md:px-12 lg:px-24 py-16 md:py-24">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
          {PROJECTS.map((project, idx) => (
            <ScrollReveal key={idx} delay={Math.min(idx * 0.04, 0.5)}>
              <ProjectCard
...
/* ── Main projects section (home page) ── */
    ...
    <div className="max-w-[1400px] mx-auto px-6 md:px-12 lg:px-24 py-28 md:py-36 relative z-10">
      <ScrollReveal>
        <div className="flex items-center gap-3 mb-4">
          ...
          <span className="font-mono text-[11px] tracking-[0.2em] uppercase" style={{ color: 'var(--accent)' }}>Selected work</span>
        </div>
      </ScrollReveal>
      <ScrollReveal delay={0.08}>
        <h2 className="font-display text-3xl md:text-5xl font-700 mb-20" style={{ color: 'var(--text)' }}>
          Things I've <span className="font-serif italic" style={{ color: 'var(--accent)' }}>built</span>
        </h2>
      </ScrollReveal>

      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 relative z-10">
        {PROJECTS.slice(0, 3).map((project, idx) => (
          <ScrollReveal key={idx} delay={0.12 + idx * 0.1}>
      ...
      <ScrollReveal delay={0.5}>
        <div className="flex justify-center mt-20">
          <button
            onClick={onShowAll}
            className="group relative inline-flex items-center gap-4 px-8 py-4 rounded-full border transition-all duration-400 hover-lift"
            style={{ borderColor: 'var(--accent-border)', color: 'var(--accent)' }}
